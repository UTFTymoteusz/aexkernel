...
int  pcuart_open (int fd, dev_fd_t* file);
int  pcuart_read (int fd, dev_fd_t* file, uint8_t* buffer, int len);
int  pcuart_write(int fd, dev_fd_t* file, uint8_t* buffer, int len);
void pcuart_close(int fd, dev_fd_t* file);
long pcuart_ioctl(int fd, dev_fd_t* file, long code, void* mem);

struct dev_char_ops pcuart_ops = {
    .open  = pcuart_open,
    .read  = pcuart_read,
    .write = pcuart_write,
    .close = pcuart_close,
...
void pcuart_1_3_irq() {
    uint8_t c;
    if (pcuart_present[1]) {
        while (inportb(pcuart_ioports[1] + REG_LSTATUS) & 1) {
            c = inportb(pcuart_ioports[1] + REG_DATA);
            if (c == 127)
                c = '\b';

            cbufm_write(&(pcuart_buffers[1]), &c, 1);
        }
    }
    if (pcuart_present[3]) {
        while (inportb(pcuart_ioports[3] + REG_LSTATUS) & 1) {
            c = inportb(pcuart_ioports[3] + REG_DATA);
            if (c == 127)
                c = '\b';
                
            cbufm_write(&(pcuart_buffers[3]), &c, 1);
        }
    }
}
...
int pcuart_open(UNUSED int fd, UNUSED dev_fd_t* file) {
    return 0;
}

int pcuart_read(int fd, UNUSED dev_fd_t* file, uint8_t* buffer, int len) {
    if (len > 64)
        len = 64;
...
int pcuart_write(int fd, UNUSED dev_fd_t* file, uint8_t* buffer, int len) {
...
void pcuart_close(UNUSED int fd, UNUSED dev_fd_t* file) {

}

long pcuart_ioctl(int fd, UNUSED dev_fd_t* file, long code, void* mem) {
    switch (code) {
